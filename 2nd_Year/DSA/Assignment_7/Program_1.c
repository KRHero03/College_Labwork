...
void insertElement(n node,char d){
	char val=node->data;
	if(d>val){
		if(node->right==NULL){
			...
			temp->data=d;
			node->right=temp;
			printf("%c inserted in Binary Search Tree! \n",d);
			return;
		}else{
		...
		printf("%c is already present in the Binary Search Tree! \n",d);
		return;
	}
...
void searchElementUtil(n node,char d){
	if(node==NULL){
		printf("%c is not present in Binary Search Tree!");
		return;
	}
	char val=node->data;
	if(val==d){
		printf("%c has been found in Binary Search Tree!");
		return;
	}else if(d<val){
