...
void findNodeCount(n node,int *d){
	if(root==NULL){
		printf("Binary Search Tree has not been created! \n");
		return;
	}
	...
	*d = *d + 1;
	findNodeCount(node->left,d);	
	findNodeCount(node->right,d);
}
...
void clearScreen(){
	...
	printf("6. Find No. of Elements in Tree\n");
	...
	while(t!=8){
		switch(t){
			...
			case 6:
				printf("\n");
				int d=0;
				findNodeCount(root,&d);
				printf("No. of Elements in Binary Search Tree:- %d \n\n",d);
				break;
			case 7:
				clearScreen();
				break;
