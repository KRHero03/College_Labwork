...
void findMax(n node){
	...
	if(node->right==NULL){
		printf("Maximum value in Binary Search Tree:- %d \n\n",node->data);
		return;
	}
	findMax(node->right);	
}
...
void findMin(n node){
	...
	if(node->left==NULL){
		printf("Minimum value in Binary Search Tree:- %d \n\n",node->data);
		return;
	}
	findMax(node->left);	
}
...
void clearScreen(){
	...
	printf("6. Find Maximum and Minimum Element in Binary Search Tree\n");
