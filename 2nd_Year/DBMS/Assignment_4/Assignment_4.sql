...
SELECT station1.NAME AS "SOURCE",station2.NAME AS "DESTINATION",track.DISTANCE 
    FROM station station1, station station2, track 
    WHERE station1.stcode= track.stcode1 AND station2.stcode=track.stcode2;

SELECT station1.NAME AS "SOURCE",station2.NAME AS "DESTINATION",track.DISTANCE 
    FROM station station1, station station2, track 
    WHERE station1.stcode= track.stcode1 AND station2.stcode=track.stcode2 AND track.DISTANCE < 20;

SELECT DISTINCT(id) 
    FROM trainhalts NATURAL JOIN station 
    WHERE station.name = 'GHATKOPAR';

SELECT train.NAME AS "NAME" 
...
SELECT train.NAME AS "NAME" 
    FROM train,trainhalts,station 
    WHERE train.id = trainhalts.id AND trainhalts.stcode = station.stcode AND trainhalts.seqno<6 AND station.name='THANE';


 SELECT SOURCE,DESTINATION,DISTANCE
    FROM(
        SELECT station1.NAME AS "SOURCE",station2.NAME AS "DESTINATION",row_number() OVER ( ORDER BY track.distance DESC) rn ,track.DISTANCE AS "DISTANCE"
        FROM  station station1, station station2, track  
        WHERE station1.stcode= track.stcode1 AND station2.stcode=track.stcode2)
    WHERE rn=1;
    
SELECT ID,timeout
    FROM (  SELECT th.*, row_number() OVER (ORDER BY th.timeout DESC) rn 
                FROM trainhalts th 
                WHERE th.timeout IS NOT NULL)
    WHERE rn = 2;
